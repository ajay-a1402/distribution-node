...
module.exports.CreateDailyUpdates = async(req, res) => {
    ...
    if (isNull(body.tc)) return ReE(res, { message: 'tc is required!' }, HttpStatus.BAD_REQUEST);
    if (isNull(body.batch)) return ReE(res, { message: 'batch is required!' }, HttpStatus.BAD_REQUEST);
    if (isNull(body.regno)) return ReE(res, { message: 'regno is required!' }, HttpStatus.BAD_REQUEST);
    if (isNull(body.Idcard)) return ReE(res, { message: 'idcard is required!' }, HttpStatus.BAD_REQUEST);
    if (isNull(body.Marksheet)) return ReE(res, { message: 'Marksheet is required!' }, HttpStatus.BAD_REQUEST);
    if (isNull(body.coursecertificate)) return ReE(res, { message: 'coursecertificate is required!' }, HttpStatus.BAD_REQUEST);
    if (isNull(body.sportscertificate)) return ReE(res, { message: 'sportscertificate is required!' }, HttpStatus.BAD_REQUEST);
...
module.exports.DeleteDailyUpdates = async(req, res) => {
    ...
    if (err) return ReE(res, err, HttpStatus.INTERNAL_SERVER_ERROR)
    if (!dailyUpdates) {
        return ReE(res, { message: 'Datas not found1' },
